...
/** Where the file physically lives; only shown when the backend recorded it. */
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Badge tone="neutral" size="sm">
                {STORAGE_LABEL[provider] ?? provider}
              </Badge>
            </TooltipTrigger>
            <TooltipContent>
              Stored in {STORAGE_LABEL[provider] ?? provider}. Deleting this material removes the
              file from there too.
            </TooltipContent>
          </Tooltip>
        )}
        ...
        <label className="flex cursor-pointer items-start gap-2.5 rounded-lg border border-border px-3 py-2.5">
          ...
          <span className="text-sm">
            ...
            <span className="mt-0.5 block text-xs text-muted-foreground">
              Removes the LMS entry but leaves the file in{' '}
              {deleting?.storage_provider
                ? (STORAGE_LABEL[deleting.storage_provider] ?? deleting.storage_provider)
                : 'storage'}
